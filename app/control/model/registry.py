...
MODELS: tuple[ModelSpec, ...] = (
    ...
    # === grok-4.5 compatibility aliases =======================================
    # 客户端常请求 grok-4.5；当前上游对外稳定入口映射到 4.3 console / 4.20 prefer_best。
    ModelSpec("grok-4.5",                               ModeId.CONSOLE,  Tier.BASIC, Capability.CONSOLE_CHAT, True, "Grok 4.5 (alias -> console)"),
...
_ALIASES: dict[str, str] = {
    ...
    "grok-4.5-latest-console": "grok-4.5-console",
}
...
def get(model_name: str) -> ModelSpec | None:
    ...
    return _BY_NAME.get(_normalize_model_name(model_name))


def resolve(model_name: str) -> ModelSpec:
    """Return the spec for *model_name*; raise ``ValueError`` if unknown."""
    spec = _BY_NAME.get(_normalize_model_name(model_name))
    if spec is None:
